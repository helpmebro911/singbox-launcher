...
| Загрузка / мерж / валидация | `ui/wizard/template/loader.go`, `vars_resolve.go`, `substitute.go`, `template_validate.go` |
...
| UI | `ui/wizard/wizard.go`, `ui/wizard/tabs/settings_tab.go`, `ui/wizard/tabs/rules_tab.go` |
...
| Тесты | `ui/wizard/models/wizard_settings_migrate_test.go`, `ui/wizard/template/vars_resolve_test.go`, `template_validate_test.go`, `substitute_test.go`, `apply_template_test.go` |
...
4. Старый state с **`enable_tun_macos`** без **`vars.tun`** → после Load в модели есть **`tun`**.
5. **Reset** снимает override (до Save ключ может отсутствовать в файле).
6. Значение **clash_secret** не логируется при штатной работе (CONSTITUTION).
