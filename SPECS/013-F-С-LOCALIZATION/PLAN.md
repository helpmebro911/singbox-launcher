# План: локализация интерфейса (выбор языка, LTR; RTL — отдельная задача)
...
### Runtime switching: частичный (только LTR)
...
### RTL: вне scope этой задачи

Полная поддержка RTL-языков (иврит, арабский и др.) не входит в 013-план. Для них потребуется:

- отдельная проработка схем выравнивания (использование `TextAlignTrailing` вместо `TextAlignLeading` там, где это нужно);
- пересмотр порядка «иконка/текст» на кнопках и лейблах;
- местами — зеркалирование контейнеров (layout mirroring) для основных блоков UI.

Это фиксируем как будущую задачу SPECS (отдельная папка), чтобы не смешивать архитектурные изменения layout’а с базовой локализацией строк.
